...
set "_g_inf_=[GAME_INFO]"
set "_g_cln_=[GAME_CLEANUP]"
...
NET SESSION >nul 2>&1
if %errorlevel% neq 0 (
	echo %_g_exc_% This script requires administrative privileges. Please run as administrator.
	pause
	exit /b 1
)
...
echo %_g_exc_% Invalid choice! Please enter a valid number.
pause
goto menu
...
if exist "%buildPath%" (
	echo %_g_cln_% Deleted folder: build
	rd /s /q "%buildPath%"
)
mkdir "%buildPath%"
cd "%buildPath%"
...
if not exist "%buildPath%" (
	...
	if %errorlevel% neq 0 (
		echo %_cm_exc_% Build failed because Game.exe did not get generated properly.
	) else (
...
echo %_g_inf_% Running vs clean...
if not exist "%vsPath%" (
	if not exist "%outPath%" (
		echo %_g_cln_% There are no Visual Studio folders to remove.
		pause
		goto menu
	)
)
...
if exist "%vsPath%" (
	echo %_g_cln_% Deleted folder: .vs
	rd /s /q "%vsPath%"
)
echo "%outPath%"
if exist "%outPath%" (
	echo %_g_cln_% Deleted folder: out
	rd /s /q "%outPath%"
)
...
if not exist "%buildPath%" (
	if not exist "%documentsPath%" (
		echo %_g_cln_% There are no game folders to remove.
		pause
		goto menu
	)
)
...
if exist "%documentsPath%" (
	echo %_g_cln_% Deleted folder: Documents/Game
